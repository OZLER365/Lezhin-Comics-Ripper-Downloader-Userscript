// ==UserScript==
// @name         All Lezhin,Beltoon,Kakao Webtoon,Toptoon(JP) Ripper(All Languages)
...
// @grant        unsafeWindow
// @grant        GM_download
// @downloadURL https://update.greasyfork.org/scripts/563063/All%20Lezhin%2CBeltoon%2CKakao%20Webtoon%2CToptoon%28JP%29%20Ripper%28All%20Languages%29.user.js
// @updateURL https://update.greasyfork.org/scripts/563063/All%20Lezhin%2CBeltoon%2CKakao%20Webtoon%2CToptoon%28JP%29%20Ripper%28All%20Languages%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    const state = {
        ...
        seenHashes: new Set(), // SHA-256 Cryptographic Hash tracking
    ...
    targetWindow.URL.createObjectURL = function(blob) {
        ...
        // FIX: Prevent the script from capturing its own exported PNG blobs!
        if (state.isDownloading) return url;

        try {
            // Absolute zero size restrictions, capturing everything
            if (blob instanceof Blob) {
                state.urlToBlob.set(url, blob);

                // If site doesn't use data-cut-index, we capture sequentially
                if (document.querySelectorAll('[data-cut-index]').length === 0) {

                    const total = getTotalPanels();
                    // Hard cap: Stop accepting background blobs if we already have the full chapter
                    if (total && state.pageRegistry.size >= total) return url;

                    computeSHA256(blob).then(hash => {
                        // Strict SHA-256 duplicate prevention
                        if (!state.seenHashes.has(hash)) {
                            state.seenHashes.add(hash);

                            const pageNum = state.pageRegistry.size + 1;
                            // Double check the cap again since hashing is asynchronous
    ...
    function syncPagesWithDOM() {
        if (state.isDownloading) return; // Prevent DOM sync from breaking arrays during download
    ...
    // --- 6. Unified Flow: Smart Auto-Scroll ---
    ...
    async function downloadToFolder() {
        ...
        state.isDownloading = true; // Activating this prevents the hook from catching our exports
